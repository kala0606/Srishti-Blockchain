...
class Chain {
    ...
    async handleNodeJoin(tx, block) {
        ...
        // 🔍 DEBUG: Log entry to handleNodeJoin
        console.log('🔍 [Chain.handleNodeJoin] Processing NODE_JOIN:', {
            nodeId,
            nodeName: tx.name,
    ...
    async recalculateKarmaBalances() {
        ...
        // Get all karma-related transactions
        ...
        // Process KARMA_UBI transactions
        for (const tx of karmaUbiEvents) {
            if (tx.recipient && tx.payload?.amount && tx.payload.amount > 0) {
                const recipient = tx.recipient;
                const amount = tx.payload.amount;
        ...
        // Process KARMA_TRANSFER transactions
        for (const tx of karmaTransferEvents) {
            if (tx.sender && tx.recipient && tx.payload?.amount && tx.payload.amount > 0) {
                const sender = tx.sender;
                const recipient = tx.recipient;
                const amount = tx.payload.amount;
                ...
                // Check if sender has sufficient balance (for validation)
                if (balances[sender] >= amount) {
                    balances[sender] -= amount;
                    balances[recipient] += amount;
                } else {
                    console.warn(`⚠️ KARMA_TRANSFER validation: ${sender} has insufficient balance (${balances[sender]} < ${amount}) at transaction time`);
                    // Still process the transfer (chain state is authoritative)
                    balances[sender] -= amount;
                    balances[recipient] += amount;
                }
            }
        ...
        // Persist to storage
        if (this.storage) {
            await this.storage.saveMetadata('karma_balances', balances);
        }
    ...
    /**
     * Validate karma balance integrity
     * Checks that current balances match what would be calculated from transactions
     * @returns {Promise<{valid: boolean, discrepancies: Array, recalculated: Object}>}
     */
    async validateKarmaBalances() {
        const recalculated = await this.recalculateKarmaBalances();
        const current = this.state.karmaBalances || {};
        const discrepancies = [];
        
        // Check all nodes in current balances
        for (const nodeId in current) {
            const currentBalance = current[nodeId] || 0;
            const recalculatedBalance = recalculated[nodeId] || 0;
            
            // Allow small floating point differences
            const diff = Math.abs(currentBalance - recalculatedBalance);
            if (diff > 0.01) {
                discrepancies.push({
                    nodeId,
        ...
        // Check for nodes in recalculated but not in current
        for (const nodeId in recalculated) {
            if (!(nodeId in current)) {
                discrepancies.push({
                    nodeId,
                    current: 0,
                    recalculated: recalculated[nodeId],
                    difference: recalculated[nodeId]
                });
            }
        }
        ...
        if (!valid) {
            console.warn(`⚠️ Karma balance validation found ${discrepancies.length} discrepancies:`);
            discrepancies.forEach(d => {
                console.warn(`   ${d.nodeId}: current=${d.current}, recalculated=${d.recalculated}, diff=${d.difference}`);
            });
        } else {
            console.log('✅ Karma balance validation passed - all balances match blockchain');
        }
    ...
    async loadState() {
        ...
        try {
            ...
            // ═══════════════════════════════════════════════════════════════
            // CRITICAL: Recalculate karma balances from blockchain transactions
            // instead of loading from storage (which may be stale)
            // ═══════════════════════════════════════════════════════════════
            if (this.blocks.length > 0) {
                await this.recalculateKarmaBalances();
            } else {
                // No blocks yet, initialize empty balances
                this.state.karmaBalances = {};
            }
    ...
    async replaceChain(newBlocks) {
        ...
        // ═══════════════════════════════════════════════════════════════
        // CRITICAL: Recalculate karma balances from all transactions
        // This ensures balances are accurate after chain replacement
        // ═══════════════════════════════════════════════════════════════
        await this.recalculateKarmaBalances();

        // 🔍 DEBUG: Log state after rebuilding
        console.log('🔍 [Chain.replaceChain] State rebuilt from transactions:');
        console.log('  - Total blocks processed:', this.blocks.length);
        console.log('  - nodeRoles:', this.state.nodeRoles);
        console.log('  - institutions (verified):', Object.keys(this.state.institutions).length);
        console.log('  - institutions (pending):', Object.keys(this.state.pendingInstitutions).length);
        console.log('  - karma balances:', Object.keys(this.state.karmaBalances).length, 'nodes');
